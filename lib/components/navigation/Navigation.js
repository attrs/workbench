...
proto.select = function (id) {
    var self = this;
    if (!id)
        return console.warn('required select node(id/instance)') && self;
    var node = self.find(id);
    if (!node)
        return console.warn('cannot find node for select', id) && self;
    var o = self.options();
    var el = $(self.dom());
    ...
    self.findall(NavItem).forEach(function (navitem) {
        if (navitem === node)
            navitem.active();
        else if (navitem.dom().contains(node.dom()))
            navitem.expand().active();
        else if (autocollapse)
            navitem.collapse().active(false);
    });
